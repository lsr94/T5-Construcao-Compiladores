...
public class GeradorC extends AlgumaGrammarBaseVisitor<Void> {
    ...
    // Método para definir o tipo da variável do arquivo AlgumaGrammar.g4
    public AlgumaGrammar tipoAlgumaGrammar(String tipoVariavel){
    ...
    // Método que realiza a conversão da variável do tipo AlgumaGrammar para a String equivalente na linguagem C
    public String converteTipoAlgumaGrammar(AlgumaGrammar tipoVariavel) {
    ...
    // Método que recebe uma string e verifica a coerência com a linguagem C
    public String verificaTipoC(String tipoString) {
        String tipoItem = null;
    ...
    // Método para definir o tipo do parâmetro de acordo com as variáveis do arquivo AlgumaGrammar.g4
    public String parametroAlgumaGrammar(AlgumaGrammar tipoP) {
    ...
    // Método que verifica uma String e retorna o parâmetro referente a leitura/ escrita na linguagem C para cada tipo de dado
    public String verificaParametroC(String tipoP) {
        String tipoParametro = null;
    ...
    // ############################################################################################# AQUI PARA BAIXO NÃO TEM NADA ALTERADO DO T4
    ...
    public void adicionaVariavelTabela(String nomeVariavel, String tipoVariavel, Token nomeToken, Token tipoToken, TipoEntrada tipoEnt, int tamArray) {
        ...
        tipoItem = tipoAlgumaGrammar(tipoVariavel);
   ...
   // Visita registro e cada uma das variáveis
    ...
    public Void visitDeclaracao_global(AlgumaGrammarParser.Declaracao_globalContext ctx) {
        ...
        if (ctx.tipo_estendido() != null) {
            tiposRetorno = tipoAlgumaGrammar(ctx.tipo_estendido().getText());
        ...
        if (ctx.parametros() != null) {
            for (var param : ctx.parametros().parametro()) {
                TabelaDeSimbolos.AlgumaGrammar paramType = tipoAlgumaGrammar(param.tipo_estendido().getText());
                    ...
                    if (tabelaRegistro.containsKey(nomeRegistro)) {
                        ...
                        for (var ident : param.identificador()) {
                            ...
                            for (int i = 0; i < variavelRegistro.size(); i += 2) {
                                ...
                                AlgumaGrammar tipoAtributo = tipoAlgumaGrammar(variavelRegistro.get(i + 1));
